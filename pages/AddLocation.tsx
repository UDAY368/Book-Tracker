...
const AddLocation: React.FC = () => {
  ...
  const handleConfirmAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalType === 'State') {
        payload.state = newItemName;
      } else if (modalType === 'District') {
        payload.district = newItemName;
      } else if (modalType === 'Town') {
        payload.district = selectedDistrict;
        payload.town = newItemName;
      } else if (modalType === 'Center') {
        payload.district = selectedDistrict;
        payload.town = selectedTown;
        payload.center = newItemName;
      }

      await api.addLocation(payload);
      await loadLocations(); // Refresh data
      
      // Select the newly added item to smooth workflow (optional, depends on UX preference)
      if (modalType === 'State') setSelectedState(newItemName);
      else if (modalType === 'District') setSelectedDistrict(newItemName);
      else if (modalType === 'Town') setSelectedTown(newItemName);
  ...
  const handleDeleteItem = async (type: string, name: string) => {
    ...
    if (type === 'State') {
        ...
        payload.state = selectedState; // Ensure context
    ...
    if (type === 'State' && selectedState === name) { 
        setSelectedState(null); setSelectedDistrict(null); setSelectedTown(null); 
    }
    if (type === 'District' && selectedDistrict === name) { 
        setSelectedDistrict(null); setSelectedTown(null); 
    }
    if (type === 'Town' && selectedTown === name) { 
  ...
  return (
    <div className="space-y-6 max-w-full pb-20 animate-in fade-in duration-500">
      
      {/* Page Header */}
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h2 className="text-2xl font-bold text-slate-800">Location Management</h2>
           <p className="text-slate-500 text-sm mt-1">Configure the hierarchy of States, Districts, Towns, and Centers.</p>
        </div>
        <div className="flex items-center gap-2 text-sm text-slate-500 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
            <span className="flex items-center"><Map size={14} className="mr-1"/> Hierarchy Mode</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
           ...
           <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all scale-100">
               <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold text-slate-800">Add New {modalType}</h3>
